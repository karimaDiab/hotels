...
class Check extends MY_Controller {
 ...
 if((!$this->db->table_exists('model_check')))
 {
   $this->db->query("CREATE TABLE `model_check` (
 ...
 $newDate = date("Ymd", strtotime($originalDate));
        ...
        for ($index = 1; $index < 34; $index++) {
             ...
             $newDate = date("Ym01", strtotime($originalDate)+($index*60*60*24*31));
             $data = array(
                    'catid' => 1,
                    'text1' =>288,
                    'text3' => "0".(406+$index),
                  
                    'text2' => "ALGHANIM SONS GROUP GENERAL TRADING & Contracting Co",
...
////echo "<br>";
         ...
         ////    $result = $this->db->insert('model_check', $data);
...
/////exit;
    ...
    function sum_bill($catid) {
        ...
        $query = $this->db->get('model_check')->row_array();
        if (!$query['text1'])
            $query['text1'] = 0;
        return $query['text1'];
    }
     ...
     public function updateok($id) {
         ...
         $query = $this->db->get_where('model_check', array('id' => $id));
        $data['show'] = $query->row_array();
        
        
          
                    $catid=2;
                    if(  $data['show']['text30']==3 or $data['show']['text30']==4) $catid=5;
                    
               
                      $dataadd = array(
                    'catid' =>$catid,
                    'text2' => "دفع من الشيكات  : ".$data['show']['text2'] ."وذلك عن ".$data['show']['text12'] ,
                    'text3' => '',
                    'text1' => $data['show']['text1'],
                    'text11' => $data['show']['text11'],
                    'text12' => $data['show']['text12'],
                    'text13' => $data['show']['text13'],
                    'text30' =>$data['show']['text30'],
        ...
        $this->booking->Sms_send("55544445","  دفع من الشيكات  :  "
              ...
              $edit_data = array(
                ...
                $result = $this->db->where('id', $id)->update('model_check', $edit_data);
                
                   $this->session->set_flashdata('msg', 'تم صرف الشيك    بفضل الله');
        redirect(base_url() . 'ltef/check');
    }
    ...
    public function all($mon = '', $catid = '', $text30= '') {
        ...
        if ($mon)
           ...
           $data['all_all'] =  $this->sum_bill("1  ") + $this->sum_bill('4') + $this->sum_bill('6');
        $data['all_bill'] = ($this->sum_bill("1  ") + $this->sum_bill('4') + $this->sum_bill('6')) - ($this->sum_bill('2  and text3!=1') + $this->sum_bill('3'));



    
       
  $data['all_billmon'] = $this->sum_bill("1   and  `dateadd` LIKE '%". substr( $this->data_day,0,6)."%'");
        ;

         $data['all_bill1'] = $this->sum_bill('1    ' );
        $data['all_bill2'] = $this->sum_bill('2  and text3!=1  ' );
        $data['all_bill5'] = $this->sum_bill('5 ' );
              $data['all_bill8'] = $this->sum_bill('8  ' );
      $whr = "  text3!=1 ";
         if (isset($text30) and $text30){
          ...
          if ($catid!='all' and $catid)
             $whr = " $whr and  catid='$catid'";
                ...
                $whr = " $whr and  catid!='5'  and  catid!='8'";
       if ($catid=='all' )
                  $whr = " text3!=1";
          if($catid=='mon')      $whr = "  `dateadd` LIKE '%". substr( $this->data_day,0,6)."%' and  catid!='5'  and  catid!='8'";
        $this->db->from('model_check');
        if ($whr)
            $this->db->where($whr);
        $max = 10000;
...
//echo print_r($this->db->last_query());
        ...
        $this->db->from('model_check');
           if($catid==8 or $catid=='all') $this->db->order_by('text3', 'desc');
...
//$this->db->limit($max, ($max*$page)-$max);
     ...
     public function updatebalc($mon = '', $catid = '') {
        ...
        foreach ($data['all_groups'] as $row) {
            ...
            $this->db->where('id', $row['id'])->update('model_check', array(
                'text5' => $totall));
    }
    ...
    public function stat($mon = '', $catid = '') {
        ...
        $query = $this->db->query("SELECT dateadd FROM `model_check` WHERE `dateadd` LIKE '%$this->thismon%' group by dateadd ORDER BY `dateadd` ASC");
    ...
    public function add($id=100000000000) {
        $add = FALSE;
                ...
                if ($result) {
                    if ($this->input->post('cash') and $this->input->post('endbill')) {
                       ...
                       /// if ($this->booking->data_day(13) != $this->data_day)
                        ...
                        redirect(base_url('ltef/'.$this->thispage.'/endbill/' . $this->data_day));
                    } else {
        ...
        if (!$add) {
                ...
                $query = $this->db->query("SELECT * FROM `model_check` ORDER BY `text3` DESC limit 1");
...
//$data['cats_select']  = $this->db->get()->result_array(); 
    ...
    public function add_another_db() {
        ...
        mysqli_query($mysqli22, "DELETE FROM model_check2 WHERE dateadd='" . $this->booking->data_day(13) . "' and text30='$ssss'");
        $this->db->from('model_check');
        $this->db->order_by('id', 'desc');
        $this->db->where('catid=5  and dateadd= ' . $this->data_day . " and `text2` not LIKE '%knet%'");
        $this->db->limit(1);
        $rowlast = $this->db->get()->row_array();
        $cccc = ( "insert model_check2 set name='" . $this->data_day . "',dateadd='" . $this->booking->data_day(13) . "',counter='1',catid='1',text30='$ssss',text1='" . $rowlast['text1'] . "',text2='    exprot.   " . $this->data_day . " cash',text3='0',catid2='0',text4='',text5='',text6='',text7='',text8='',text9='',text10='',text11='',text12='',text13='',text14='',text15='',text16='',text17='',text18='',text19='',text20='' ,text21='' ,text22='' ,text23='' ,text24='' ,text25='' ,text26='' ,text27='' ,text28='',text29=''  ");
...
//$this->db->limit($max, ($max*$page)-$max);
    ...
    public function del($id) {
        ...
        $this->db->where('id', $id);
        $result = $this->db->delete('model_check');
        if (count($result) > 0) {
    ...
    public function show($id) {
       ...
       /// echo $data['show']['text30'];
   if($data['show']['text30']) $data['show']['text30'] = $this->nao2[$data['show']['text30']];
        $add = FALSE;
 ...
 $explode = explode('.',  $data['show']['text1']);
     
        $data['first_sum'] =$this->booking->numtoarb($explode[0]);
...
if($this->check_username( $data['show']['text2']))
{
    
     $this->load->view( '../controllers/ltef/views/' . $this->thispage . '/printcheck', $data); 
} else {
      $this->load->view( '../controllers/ltef/views/' . $this->thispage . '/printcheck', $data);
}
...
{
$exp = "/^[\ ضصثقفغعهخحجدشسيبلاتنمكطئءؤرلاىةوزظذإأا َ ً ُ ٌ ِ ٍ ْ ّ.-0123456789]{3,100}$/";
