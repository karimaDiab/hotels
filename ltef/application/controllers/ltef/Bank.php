...
class Bank extends MY_Controller {
 ...
 if((!$this->db->table_exists('model_bank')))
 {
   $this->db->query("CREATE TABLE `model_bank` (
  ...
  `text30` varchar(255) DEFAULT NULL,
    ...
    function sum_bill($catid) {
        ...
        $query = $this->db->get('model_bank')->row_array();
        if (!$query['text1'])
            $query['text1'] = 0;
        return $query['text1'];
    }
        ...
        $data['all_bill5'] = $this->sum_bill('5 and ' . $whr);
         ...
         if (isset($text30) and $text30){
            ...
            $data['all_billn3'] = ($data['all_bill1n']) - ( $data['all_bill2n']+$data['all_bill5n'] );
                     ...
                     $data['all_billn51'] = $this->sum_bill('1 and  text30=5  and ' . $whr);
                     $data['all_bill55'] = $this->sum_bill('5 and text30=5  and ' . $whr);
          if ($catid!='all' and $catid)
             $whr = " $whr and  catid='$catid'";
        $this->db->from('model_bank');
        if ($whr)
            $this->db->where($whr);
        $max = 10000;
...
//$this->db->limit($max, ($max*$page)-$max);
     ...
     public function updatebalc($mon = '', $catid = '') {
        ...
        foreach ($data['all_groups'] as $row) {
            ...
            $this->db->where('id', $row['id'])->update('model_bank', array(
                'text5' => $totall));
    }
    ...
    public function stat($mon = '', $catid = '') {
        ...
        $query = $this->db->query("SELECT dateadd FROM `model_bank` WHERE `dateadd` LIKE '%$this->thismon%' group by dateadd ORDER BY `dateadd` ASC");
    ...
    public function add() {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $result = $this->db->insert('model_bank', $data);
                if ($result) {
...
//$data['cats_select']  = $this->db->get()->result_array(); 
            ...
            $query = $this->db->get_where('model_bank', array('id' => '100000000000'));
    ...
    public function add_another_db() {
        ...
        mysqli_query($mysqli22, "DELETE FROM model_bank2 WHERE dateadd='" . $this->booking->data_day(13) . "' and text30='$ssss'");
        $this->db->from('model_bank');
        $this->db->order_by('id', 'desc');
        $this->db->where('catid=5  and dateadd= ' . $this->data_day . " and `text2` not LIKE '%knet%'");
        $this->db->limit(1);
        $rowlast = $this->db->get()->row_array();
        $cccc = ( "insert model_bank2 set name='" . $this->data_day . "',dateadd='" . $this->booking->data_day(13) . "',counter='1',catid='1',text30='$ssss',text1='" . $rowlast['text1'] . "',text2='    exprot.   " . $this->data_day . " cash',text3='0',catid2='0',text4='',text5='',text6='',text7='',text8='',text9='',text10='',text11='',text12='',text13='',text14='',text15='',text16='',text17='',text18='',text19='',text20='' ,text21='' ,text22='' ,text23='' ,text24='' ,text25='' ,text26='' ,text27='' ,text28='',text29=''  ");
...
//$this->db->limit($max, ($max*$page)-$max);
    ...
    public function del($id) {
        ...
        $this->db->where('id', $id);
        $result = $this->db->delete('model_bank');
        if (count($result) > 0) {
    ...
    public function edit($id) {
        ...
        $query = $this->db->get_where('model_bank', array('id' => $id));
        $data['edit'] = $query->row_array();
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $result = $this->db->where('id', $id)->update('model_bank', $edit_data);
