...
class Salry extends MY_Controller {
 ...
 if((!$this->db->table_exists('model_salry')))
 {
   $this->db->query("CREATE TABLE `model_salry` (
  ...
  `text30` varchar(255) DEFAULT NULL,
    ...
    function sum_bill($catid) {
        ...
        $query = $this->db->get('model_salry')->row_array();
        if (!$query['text1'])
            $query['text1'] = 0;
        return $query['text1'];
    }
...
//$this->db->limit($max, ($max*$page)-$max);
     ...
     public function updatebalc($mon = '', $catid = '') {
        ...
        foreach ($data['all_groups'] as $row) {
            ...
            $this->db->where('id', $row['id'])->update('model_salry', array(
                'text5' => $totall));
    }
    ...
    public function stat($mon = '', $catid = '') {
        ...
        $query = $this->db->query("SELECT dateadd FROM `model_salry` WHERE `dateadd` LIKE '%$this->thismon%' group by dateadd ORDER BY `dateadd` ASC");
    ...
    public function add() {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $result = $this->db->insert('model_salry', $data);
                 if($this->input->post('addbill'))
                {
                    ...
                    if($this->input->post('text30')==3 or $this->input->post('text30')==4) $catid=5;
                      ...
                      $data = array(
                    'catid' =>$catid,
                    'text2' => "دفع من المعتمدين  : ".$this->input->post('text2') ,
                    'text3' => '',
                    'text1' => $this->input->post('text1'),
                    'text11' => $this->input->post('text11'),
                    'text12' => $this->input->post('text12'),
        ...
        $this->booking->Sms_send("55544445","  دفع من المعتمدين  :  "
                . "\n"
...
//$data['cats_select']  = $this->db->get()->result_array(); 
            ...
            $query = $this->db->get_where('model_salry', array('id' => '100000000000'));
  ...
  function sum_bill2($catid) {
    ...
    public function add_another_db() {
        ...
        mysqli_query($mysqli22, "DELETE FROM model_salry2 WHERE dateadd='" . $this->booking->data_day(13) . "' and text30='$ssss'");
        $this->db->from('model_salry');
        $this->db->order_by('id', 'desc');
        $this->db->where('catid=5  and dateadd= ' . $this->data_day . " and `text2` not LIKE '%knet%'");
        $this->db->limit(1);
        $rowlast = $this->db->get()->row_array();
        $cccc = ( "insert model_salry2 set name='" . $this->data_day . "',dateadd='" . $this->booking->data_day(13) . "',counter='1',catid='1',text30='$ssss',text1='" . $rowlast['text1'] . "',text2='    exprot.   " . $this->data_day . " cash',text3='0',catid2='0',text4='',text5='',text6='',text7='',text8='',text9='',text10='',text11='',text12='',text13='',text14='',text15='',text16='',text17='',text18='',text19='',text20='' ,text21='' ,text22='' ,text23='' ,text24='' ,text25='' ,text26='' ,text27='' ,text28='',text29=''  ");
...
//$this->db->limit($max, ($max*$page)-$max);
    ...
    public function del($id) {
        ...
        $this->db->where('id', $id);
        $result = $this->db->delete('model_salry');
        if (count($result) > 0) {
 ...
 public function took($id) {
         ...
         $query = $this->db->get_where('model_salry', array('id' => $id));
        $data['show'] = $query->row_array();
              ...
              $edit_data = array(
                ...
                $result = $this->db->where('id', $id)->update('model_salry', $edit_data);
