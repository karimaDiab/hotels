...
$flagscounter2['all']++;
 ...
 $result = $this->db->insert('model_ohda', $data);       
        ...
        $result = $this->db->insert('model_ohda', $data);       
            ...
            $this->add_another_db();
                       ...
                       redirect(base_url('ltef/bills2/endbill/' . $this->data_day));
           ...
           if($this->input->post('addbill'))
                {
                    ...
                    if($this->input->post('text30')==3 or $this->input->post('text30')==4) $catid=5;
                      ...
                      $data = array(
                    'catid' =>$catid,
                    'text2' => "دفع من العهدة  : ".$this->input->post('text2') ,
                    'text3' => '',
                    'text1' => $this->input->post('text1'),
                    'text11' => $this->input->post('text11'),
                    'text12' => $this->input->post('text12'),
  ...
  $dbhost = "localhost"; // الخادم
        $dbuser = "kuwaityc_bookin3";  // إسم مستخدم قاعدة البيانات
...
//$this->db->limit($max, ($max*$page)-$max);
  ...
  $query = $this->db->get_where('model_ohda', array('id' => $id));
        ...
        $this->db->where('id', $id);
        $result = $this->db->delete('model_ohda');
        if (count($result) > 0) {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $edit_data = $this->security->xss_clean($edit_data);
                 ...
                 $this->db->where('id', $id);$result =$this->db->update('model_ohda', $edit_data);
    ...
    public function endbill($day) {
       ...
       /// $this->load->view('../controllers/ltef/views/' . $this->thispage . '/all_booking', $data);
    }
