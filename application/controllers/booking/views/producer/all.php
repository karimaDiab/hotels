...
<div class="row">
    ...
    <td><?= $this->booking->tissme_show($row['dateadd']); ?></td>
                                    ...
                                    <td>     <a title="Edit" class="update btn btn-sm btn-primary pull-right" href="<?= base_url('booking/' . $this->thispage . '/show/' . $row['id']) ?>"><i class="material-icons">مشاهدة</i></a>
                                    </td>

  <td>     <a title="Edit" class="update btn btn-sm btn-primary pull-right" href="<?= base_url('booking/' . $this->thispage . '/add/' . $row['id']) ?>"><i class="material-icons">اضافة جديدة</i></a>
                                    </td>
...
<?php if ($all_count > 2): ?>
    ...
    <?php for ($count = 1; $count < $all_count; $count++): ?>
                                    <li><a href="<?= base_url('booking/' . $this->thispage . '/all/' . $count.'/'.$producerid); ?>"><?= $count ?></a></li>
