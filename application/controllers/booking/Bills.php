    ...
    public function all($mon = '', $catid = '') {
        ...
        if ($whr)
            $this->db->where($whr);
             ...
             $max = 15; if (!$this->session->userdata('group'))    $max = 7;
         if($this->session->userdata('group')) $max = 10000;
...
//$this->db->limit($max, ($max*$page)-$max);
    ...
    public function updatebalc($mon = '', $catid = '') {
        ...
        foreach ($data['all_groups'] as $row) {
            ...
            $this->db->where('id', $row['id']);$this->db->update('model_billshawly', array(
                'text5' => $totall));
        }
...
$flagscounter2['all']++;
 ...
 $result = $this->booking->insert('model_billshawly', $data);       
    ...
    public function add() {
      ...
      $query = $this->db->get_where('model_billshawly', array('dateadd' => $this->data_day,'catid'=>'5','text2'=>'كاش'));
        $data['show'] = $query->row_array();
               
               if(isset($data['show']['catid'])==5)
               {
                ...
                if ($result) {
                    if ($this->input->post('cash') and $this->input->post('endbill')) {
                       ...
                       redirect(base_url('booking/bills/endbill/' . $this->data_day));
                        ...
                        redirect(base_url('booking/bills/'));
                    }
                }
            }
        }
...
//     $dbname = "mktba2018"; // إسم قاعدة البيانات
        if (@mysqli_connect($dbhost, $dbuser, $dbpass, $dbname)) {
            ...
            mysqli_query($mysqli22, "SET character_set_server='utf8'"); // هنا تم اضافة كود الترميز الخاص بالغة العربية
            mysqli_query($mysqli22, "SET NAMES utf8");
            mysqli_query($mysqli22, "DELETE FROM model_billshawly2 WHERE dateadd='" . $this->booking->data_day(13) . "' and text30='$ssss' and catid='1'");
            $this->db->from('model_billshawly');
            $this->db->order_by('id', 'desc');
            $this->db->where('catid=5  and dateadd= ' . $this->data_day . " and `text2` not LIKE '%knet%'");
            $this->db->limit(1);
            $rowlast = $this->db->get()->row_array();
            $cccc = ( "insert model_billshawly2 set name='" . $this->booking->data_day(13) . "',dateadd='" . $this->booking->data_day(13) . "',counter='1',catid='1',text30='$ssss',text1='" . $rowlast['text1'] . "',text2='    $ddd  " . $this->booking->data_day(13) . " كاش',text3='0',catid2='0',text4='',text5='',text6='',text7='',text8='',text9='',text10='',text11='',text12='',text13='',text14='',text15='',text16='',text17='',text18='',text19='',text20='' ,text21='' ,text22='' ,text23='' ,text24='' ,text25='' ,text26='' ,text27='' ,text28='',text29=''  ");
...
//$this->db->limit($max, ($max*$page)-$max);
            ...
            foreach ($data['all_groups'] as $row) {
                ...
                {
                     $query = $this->db->get_where('model_billshawly', array('catid' => "8",'dateadd'=> $row['dateadd']));
        $data['edit'] = $query->row_array();  
     if($data['edit']['text1']) $row['text1'] =$row['text1'] -  $data['edit']['text1'];
                ...
                $wqq = mysqli_query($mysqli22, "select * from  model_billshawly where dateadd='" . $row['dateadd'] . "' and text30='$ssssss' and catid='" . $row['catid'] . "' and text1='" . $row['text1'] . "'  and text2='$ddd " . $row['text2'] . "' order by id desc")or die("ddd");
                $num_sql = mysqli_num_rows($wqq);
                if ($num_sql == 0) {
                 
                    
                  
                    $cccc = ( "insert model_billshawly set name='',dateadd='" . $row['dateadd'] . "',counter='1',catid='" . $row['catid'] . "',text30='$ssssss',text1='" . $row['text1'] . "',text2='$ddd " . $row['text2'] . "',text3='0',catid2='" . $row['catid2'] . "',text4='',text5='',text6='',text7='',text8='',text9='',text10='',text11='',text12='',text13='',text14='',text15='',text16='',text17='',text18='',text19='',text20='' ,text21='' ,text22='' ,text23='' ,text24='' ,text25='' ,text26='' ,text27='' ,text28='',text29=''  ");
                    mysqli_query($mysqli22, $cccc);
                }
            }
...
//$this->db->limit($max, ($max*$page)-$max);
            ...
            foreach ($data['all_groups'] as $row) {
                ...
                $wqq = mysqli_query($mysqli22, "select * from  model_billshawly where dateadd='" . $row['dateadd'] . "' and text30='$ssss' and catid='" . $row['catid'] . "' and text1='" . $row['text1'] . "' and text2='$ddd " . $row['text2'] . "' order by id desc")or die("ddd");
                $num_sql = mysqli_num_rows($wqq);
                if ($num_sql == 0) {
  ...
  $query = $this->db->get_where('model_billshawly', array('id' => $id));
        ...
        $this->db->where('id', $id);
        $result = $this->booking->delete('model_billshawly');
        if (count($result) > 0) {
        ...
        if ($this->input->post('submit')) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $edit_data = $this->security->xss_clean($edit_data);
                 ...
                 $this->db->where('id', $id);$result =$this->booking->update('model_billshawly', $edit_data);
    ...
    public function endbill($day) {
        ...
        $this->db->from('model_billshawly');
        $this->db->order_by('id', 'desc');
        $this->db->where('catid=8 and dateadd= ' . $day);
        ...
        $data['all5_last'] = '0';
        if ($data['all5'] > 0) {
          if(isset($row['text1']))  $data['all5_last'] = $row['text1'];
        } //it will provide latest or last record id.


        $this->db->from('booking');
        $this->db->order_by('id', 'desc');
        $this->db->where('datetext4=' . $day . ' and knet>1 and bookedok IS NULL');
        $data['knet'] = $this->db->get()->result_array();


           $this->db->from('booking');
        $this->db->order_by('id', 'desc');
        $this->db->where('datetext4=' . $day . ' and knet>1 and bookedok IS NOT NULL');
        $data['bookedok'] = $this->db->get()->result_array();
