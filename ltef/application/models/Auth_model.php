<?php
   ...
   $userpw=md5(md5($data['password']));
	///		$query = $this->db->get_where('user', array('name' => $data[email]));
   			$query = $this->db->query("select * from user where ((pass='$userpw' and  name='".$data['email']."'  )or (newpass='$userpw' and newpass!='' and  name='".$data['email']."'  ) ) and(gruop='1'  or gruop='2' or gruop='5' or gruop='6' )");
